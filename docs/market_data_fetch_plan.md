...
- **Layer 2 incremental input gate** — closed for runner integrity. `profile_engine_30d` and `audit_layer2_30d` validate PMI history, Cleveland Fed nowcast, and aggregate EPS weekly history before classification. The gate rejects latest-only PMI history, stale/missing nowcast, stale/missing EPS weekly history, and EPS histories that cannot produce a non-null `aggregate_forward_eps_revision_direction_4w`.
