"""v2 §1D Breadth axis — classify layer.

V1 raw-label walker (``raw_label_for_day`` / ``build_raw_outputs``) plus
the V2 §1D label resolver (``resolve_v2_raw_outputs``) and its predicate
helpers. The features layer lives in ``breadth_state.py``.

V1 precedence (§6.9 ETF-proxy):
    divergent_fragile > weak_breadth > healthy_breadth > neutral_breadth > unknown

V2 §1D extension (spec line 385):
    breadth_thrust > divergent_fragile > narrowing_breadth > recovery_breadth >
    broadening_breadth > weak_breadth > healthy_breadth > neutral_breadth > unknown
"""
