...
| local `^NYICDX` dollar-index history | manual Yahoo Finance CSV export | daily | `data/raw/usd_index/nyicdx_daily.parquet` | done-live-verified | imported via `--fetch usd-index-local --usd-index-csv /Users/avinashvankadaru/Desktop/NYICDX_history.csv`; this is an optional local auxiliary series, not the spec primary `broad_usd_index`; when `--acquisition-db` is supplied, the manual CSV file is recorded in SQLite before parquet/report output |
| PMI manufacturing headline values | DBnomics primary, TradingEconomics backup | monthly | `data/raw/pmi/us_ism_pmi.parquet` | done-live-verified | available first business day of the following month at 10:00 ET; live run rejected stale DBnomics data and selected TradingEconomics for `2026-04`; when `--acquisition-db` is supplied, raw source HTML pages are recorded before parquet/report output |
| PMI services headline values | DBnomics primary, TradingEconomics backup | monthly | `data/raw/pmi/us_ism_pmi.parquet` | done-live-verified | available third business day of the following month at 10:00 ET; live run selected TradingEconomics `2026-04` after stale-primary rejection; when `--acquisition-db` is supplied, raw source HTML pages are recorded before parquet/report output |
...
| `aggregate_forward_eps_revision_direction_4w` | derived from weekly aggregate forward EPS history | weekly | no output path yet | planned | not derivable from the current captured workbook alone because it exposes quarterly historical observations plus one current snapshot, not a weekly revision time series; the Wayback backfill path now records raw CDX listings, archived workbook files, and derived timeline artifacts in SQLite when `--acquisition-db` is supplied |
...
| ICE DXY history | licensed ICE feed | daily | no output path while spec stays on `broad_usd_index` | hard-fail | still not available here as a licensed feed; do not confuse it with the separate optional local Yahoo `^NYICDX` import |
