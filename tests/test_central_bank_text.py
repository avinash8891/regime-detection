...
def test_same_date_aggregation_token_weighted_average_falls_back_to_mean_for_zero_weights() -> None:
    scored = pd.DataFrame(
        [
            {
                "release_date": dt.date(2024, 2, 1),
...
def test_same_date_aggregation_token_weighted_average_returns_nan_when_all_scores_nan() -> None:
    scored = pd.DataFrame(
        [
            {
                "release_date": dt.date(2024, 2, 1),
