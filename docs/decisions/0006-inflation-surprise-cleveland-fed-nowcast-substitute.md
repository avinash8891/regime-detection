...
- `download_cleveland_fed_nowcast_json` fetches the feed; `parse_cleveland_fed_nowcast_json` extracts, per vintage, the **last non-empty `CPI Inflation` value**, keyed to that point's chart category date. This preserves point-in-time availability and avoids leaking a settled monthly nowcast into earlier sessions.
