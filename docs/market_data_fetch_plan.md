...
| `cpi_all_items_vintages` / `CPIAUCSL` realtime observations | FRED API with realtime params | monthly vintages | `data/raw/macro_vintages/cpi_all_items_vintages.parquet` | implemented-not-live-verified | optional PIT/vintage view. The fetch workflow can write this when `--include-cpi-vintages` is enabled, but the current checkout has no `data/raw/macro_vintages/` artifact and the current macro fetch report has `include_cpi_vintages=false`; no active V2 metric depends on this file today |
...
- **CPI vintage artifact** — optional `cpi_all_items_vintages` fetch path is implemented, but no `data/raw/macro_vintages/cpi_all_items_vintages.parquet` file is present in this checkout; no active V2 metric consumes it today
