...
def test_fetch_text_result_distinguishes_network_failure_from_empty_page(
...
def test_fetch_text_result_accepts_valid_empty_page(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_fetch_text_result_raises_on_invalid_utf8(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        _common, "urlopen", lambda *_args, **_kwargs: _Response(b"\xff")
    )
