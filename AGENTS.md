...
6. **Tests run and pass locally before final verification.** Full-suite pytest verification should run on the workstation using the repository's real pytest command unless the user explicitly asks to defer to CI. Agents may still run targeted local checks while debugging, but final verification reports the local command, exit code, runtime, and relevant summary.
...
- **Full-suite verification runs locally by default.** Run the full pytest suite on the workstation for final verification unless the user explicitly asks to use GitHub Actions instead. Use targeted local checks for debugging, then report the full local command, exit code, runtime, and relevant summary.
