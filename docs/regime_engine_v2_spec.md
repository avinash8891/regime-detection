...
- `geopolitical_event` — approval-gated Group B candidate for war, sanctions, terrorism, conflict/protest shocks; generated from GPR quantitative evidence, AI-GPR context, GDELT, and HDX HAPI evidence when those live sources are available; ACLED and Uppsala/UCDP evidence is TODO pending entitled API keys/account access; GPR requires a headline `GPRD` spike before emitting a candidate, while acts/threats/persistence/article-count/monthly-country/AI-GPR evidence sets subtype, confidence, importance, suggested `window_days`, and review snippets; rendered only when the approval overlay promotes it; row-level `window_days` overrides the static `(0, 0)` manual fallback
