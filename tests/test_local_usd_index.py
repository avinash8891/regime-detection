...
def test_load_yahoo_usd_index_csv_validates_and_normalizes(tmp_path: Path) -> None:
    ...
    with csv_path.open("w", newline="") as handle:
        ...
        writer.writerow(
            ["2026-05-06", "100.5", "101.5", "100.0", "101.0", "101.0", "0"]
        )
...
def test_load_yahoo_usd_index_csv_quarantines_blank_rows_under_threshold(
    tmp_path: Path,
) -> None:
    csv_path = tmp_path / "NYICDX_history.csv"
    with csv_path.open("w", newline="") as handle:
        ...
        for i in range(100):
            ...
            writer.writerow(
                [day.isoformat(), "100.0", "101.0", "99.0", "100.5", "100.5", "0"]
            )
...
def test_run_local_usd_index_import_records_sqlite_artifact_and_outputs(
    tmp_path: Path,
) -> None:
    csv_path = tmp_path / "NYICDX_history.csv"
    with csv_path.open("w", newline="") as handle:
