...
class MonetaryPressureV2FeaturesConfig(StrictBaseModel):
    ...
    Configures the yield_change_zscore template (§2A line 2846) applied to
    ...
    # v2 §2A line 2846 — `yield_change_63d[t] = yield[t] - yield[t-63]`.
    ...
    # v2 §2A line 2842 — mean/std normalizer window ("5y"). 5y ≈ 1260
    ...
    # v2 §2A line 2857 — 21d-variant rate_shock predicate lookback. Mechanical
    # generalization of the line-2846 template using a 21d change window.
    rate_shock_lookback_days: int = Field(gt=0, default=21)

    # v2 §2A line 2853 — broad_usd_index z-score lookback. Mechanical generalization
    # of the line-2846 template applied to a USD-index level series.
...
class MonetaryPressureV2RulesConfig(StrictBaseModel):
    ...
    # §2A lines 2873-2876 tightening_pressure: yield_change_zscore_*_63d > +1.5 OR broad_usd > +1.5.
    tightening_pressure_zscore_threshold: float = Field(default=1.5, gt=0.0)
    # §2A lines 2878-2880 easing_pressure: yield_change_zscore_*_63d < -1.5 on either tenor.
    easing_pressure_zscore_threshold: float = Field(default=-1.5, lt=0.0)
    # §2A lines 2882-2884 rate_shock: abs(yield_change_zscore_21d_*) > 2.0.
...
class CentralBankTextConfig(StrictBaseModel):
    """v2 §2A central-bank-text classifier config (spec lines 2935-2942).
...
class InflationGrowthRulesConfig(StrictBaseModel):
    ...
    Defaults match the spec verbatim (§2B lines 3038-3083).
    """

    # §2B line 3039 — goldilocks "abs drift over 21d <= 0.005" (50bps).
    cpi_drift_threshold: float = Field(default=0.005, gt=0.0)
    # §2B lines 3042 / 3071 — pmi > 50.
    pmi_goldilocks_threshold: float = Field(default=50.0, gt=0.0)
    pmi_recovery_threshold: float = Field(default=50.0, gt=0.0)
    # §2B line 3058 — disinflation "pmi > 45".
    ...
    # §2B line 3048 — inflation_shock "commodity_return_63d > 0.15".
    commodity_return_threshold: float = Field(default=0.15, gt=0.0)
    # §2B line 3064 — recession_scare "spy_21d_return < -0.05".
    ...
    # Default -0.05 matches spec §2B line 3064 (ADR 0012 R2 ratified 2026-05-22:
    ...
    # §2B line 3085 — earnings_expansion "aggregate_forward_eps_revision_
    ...
    # §2B line 3089 — earnings_contraction "... < -0.02". Must be < 0
...
class InflationGrowthConfig(StrictBaseModel):
    ...
    unknown-gate staleness thresholds (§2B lines 3131-3137).
    """
    ...
    # §2B lines 3115-3128 — per-label asymmetric hysteresis days.
    deescalation_days_by_label: dict[str, int]
    default_deescalation_days: int = Field(default=0, ge=0)
    # §2B line 3134 — "CPI stale > 60 calendar days" (2× monthly cycle).
    cpi_stale_calendar_days: int = Field(default=60, ge=1)
    # §2B line 3135 — "PMI stale > 45 calendar days" (1.5× monthly cycle).
    pmi_stale_calendar_days: int = Field(default=45, ge=1)
    # §2B line 3136 — "DGS10 stale > 5 sessions".
...
class CreditFundingRulesConfig(StrictBaseModel):
    ...
    Defaults match the spec verbatim (§2C lines 3231-3253).
    """

    # §2C line 3232 — credit_calm "<0.50"
    hy_percentile_calm_max: float = Field(default=0.50, ge=0.0, le=1.0)
    # §2C line 3240 — credit_stress ">0.80"
    hy_percentile_stress_min: float = Field(default=0.80, ge=0.0, le=1.0)
    # §2C lines 3241/3249 — equities-falling threshold "< -0.05".
    spy_drop_threshold: float = Field(default=-0.05, le=0.0)
    # §2C line 3244 — funding_squeeze "> +1.5" (USD z-score 21d-change variant).
    broad_usd_zscore_funding_threshold: float = Field(default=1.5, gt=0.0)
    # §2C line 3251 — deleveraging "> 0" (USD z-score 21d-change variant).
    broad_usd_zscore_deleveraging_threshold: float = Field(default=0.0)
    # §2C line 3252 — deleveraging "realized_vol_21d_percentile_252d > 0.75".
    realized_vol_percentile_threshold: float = Field(default=0.75, ge=0.0, le=1.0)
    # §2C line 3253 — deleveraging "avg_pairwise_corr_percentile_504d > 0.75".
    correlation_percentile_threshold: float = Field(default=0.75, ge=0.0, le=1.0)
    # Opt-in calibration lever (default False matches spec §2C lines 3236-3237
    ...
    # §2C line 3203 — 504d percentile lookback ("scale-invariant predicate").
    hy_percentile_504d_lookback: int = Field(default=504, ge=20)
    # §2C lines 3204-3205 — 21d OLS slope window.
    slope_21d_lookback: int = Field(default=21, ge=5)
    # §2C line 3212 — KRE/SPY ratio 63d OLS slope window.
    slope_63d_lookback: int = Field(default=63, ge=5)
    # §2C lines 3198-3199 — 63d total-return lookback.
    total_return_lookback_days: int = Field(default=63, ge=5)
    # §2C line 3241 — spy_21d_return lookback.
    spy_return_lookback_days: int = Field(default=21, ge=5)
    # §2C line 3250 — tlt_21d_return lookback.
    tlt_return_lookback_days: int = Field(default=21, ge=5)
    # §2C lines 3217-3221 — 21d-change variant (vs §2A 63d-change variant).
    broad_usd_change_window_days: int = Field(default=21, ge=5)
    # §2C lines 3219-3220 normalizer window (~5y trading days, same as §2A).
...
class CreditFundingConfig(StrictBaseModel):
    ...
    code per spec §2C feature block (lines 3168-3226) — no yaml override.
    """

    rules: CreditFundingRulesConfig = Field(default_factory=CreditFundingRulesConfig)
    # §2C lines 3274-3284 — per-label asymmetric hysteresis days.
    deescalation_days_by_label: dict[str, int]
    # Labels not listed take this default.
    default_deescalation_days: int = Field(default=0, ge=0)
    # §2C line 3290 — NFCI weekly: "stale > 14 days (2× weekly release cycle)".
    nfci_stale_days: int = Field(default=14, ge=1)
    # §2C line 3289 — HYG/LQD/TLT stale > 5 sessions.
