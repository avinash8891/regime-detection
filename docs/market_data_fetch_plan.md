...
currently consumed by fetch paths.
...
- **Layer event-calendar rows** (`budget_week`, `election_window`, `geopolitical_event`, `global_rate_decision`) are generated as candidate evidence first, then promoted under the Group B rules. `election_window`, `budget_week`, and global-rate decisions have deterministic/official-source paths and are included by default in daily `--fetch all` / `--fetch events` runs. `geopolitical_event` now has live GPR + GDELT evidence generation plus implemented clients for ACLED and Uppsala/UCDP GED Candidate. **TODO:** ACLED and Uppsala/UCDP live pulls remain pending entitled API keys/account access; the current Gmail ACLED account can mint a token but the API endpoint denies raw data access. HDX HAPI monthly/admin aggregates are intentionally not candidate sources because their month-start dates are not daily market shock dates. All geopolitical evidence still reaches YAML only through the human approval overlay; no geopolitical source auto-promotes.
...
| Cross-asset ETF daily OHLCV: `QQQ,IWM,EFA,EEM,TLT,IEF,HYG,LQD,GLD,USO,DBC,UUP` | local imported parquet dataset; Alpaca REST remains an optional refresh path for 2016+ and Yahoo/local canonical parquet covers the 2010 warm-up extension for DBC/IEF | daily | SQLite `daily_ohlcv_rows` plus source parquet artifacts in the acquisition DB; engine-facing parquet under `data/raw/daily_ohlcv_762/` | done-live-verified | all listed cross-asset ETFs are present in the engine-facing OHLCV tree; DBC/IEF carry rows from `2009-12-31` through `2026-05-15` so the 2016+ network-fragility run has full 504-session warm-up |
| `DBC` daily OHLCV (V2 §2B commodity proxy and V2 §3 network-fragility broad-commodity sleeve) | direct Alpaca fetch for 2016+; Yahoo/local canonical parquet for 2010 warm-up extension | daily | `data/raw/daily_ohlcv_762/symbol=DBC/ohlcv.parquet` | done-live-verified | approved substitute for Bloomberg Commodity Index per V2 §2B Ambiguity Log #48; also admitted to the §3.1 fragility universe by ADR 0017's DBC/IEF paired universe amendment |
...
| `cpi_all_items` / `CPIAUCSL` | FRED API | monthly | `data/raw/macro/fred_macro_series.parquet` | done-live-verified | monthly CPI level; available after BLS CPI release each month; when `--acquisition-db` is supplied, the raw FRED JSON response is recorded before parquet/report output. **Note:** CPI October 2025 is permanently missing from BLS due to the 2025 US government shutdown (BLS footnote: "Data unavailable due to the 2025 lapse in appropriations"). The engine handles this via forward-fill (September value carries through October) and the 60-day staleness gate (ADR 0010 D4). |
| `cpi_all_items_vintages` / `CPIAUCSL` realtime observations | FRED API with realtime params | monthly vintages | `data/raw/macro_vintages/cpi_all_items_vintages.parquet` | implemented-default-on | PIT/vintage view required by V2 §2A lines 2587-2593 for first-release historical replay (audit M2 — `docs/spec_code_data_audit_2026_05_15.md` §3.2). `--include-cpi-vintages` default is now **True** in `scripts/fetch_regime_engine_v1_data.py`; consumed by `loaders.load_cpi_vintages_first_release` and routed into `inflation_growth.compute_inflation_growth_features` when the `use_first_release_cpi_when_available` config flag is True. Emits `cpi_first_release_vintage_replay` provenance row on the feature-store bias-warnings frame. **Bug fix (2026-05-19):** `_drop_null_fred_observations` now runs after merge (was before, letting old NaN rows from previous fetches survive in the vintages parquet). |
...
| Event calendar extension | generated `FOMC` / `CPI` / `NFP` YAML plus runtime `expiry_week` / `earnings_season` rules; layer event calendar inputs add official/deterministic/candidate-generated `budget_week` / `election_window` / `geopolitical_event` / `global_rate_decision` evidence | Daily event fetch auto-generates the routine scheduled rows and computes rule-derived windows at runtime. Candidate generation writes parquet evidence first; only approved/promoted rows enter YAML. GPR/AI-GPR, GDELT, ACLED, and UCDP geopolitical sources are generated and validated separately and are never auto-promoted. HDX HAPI monthly/admin aggregates are excluded from event candidate generation. `macro_event_score` includes `budget_week` / `election_window` / `global_rate_decision` in addition to `fed_week` / `cpi_week` / `nfp_week`; `geopolitical_event` is separate high-impact evidence. |
| V2 §2B commodity-index proxy (DBC) | Alpaca REST daily OHLCV for 2016+; Yahoo/local canonical parquet for 2010 warm-up extension | substitute for Bloomberg Commodity Index per Ambiguity Log #48; bias-warning row must be carried in feature-store output; also a §3.1 fragility universe member after ADR 0017's paired DBC/IEF amendment |
| V2 §2C HY/IG OAS + proxy | FRED `BAMLH0A0HYM2` (HY Master II OAS) + `BAMLC0A4CBBB` (BBB Corporate OAS), plus already-fetched `TLT`/`HYG`/`LQD` closes | real bps-level OAS produces `credit_funding_state` for 2023-05-15+; TLT-vs-HYG/LQD total-return differentials produce separate `credit_funding_state_proxy` for longer directional history (Ambiguity Log #49 + #71, ADR 0007). `credit_funding_effective_state` resolves the two classified labels for downstream rules without splicing raw series. IEF is a §3.1 duration-diversification input, not a new §2C credit/funding proxy; do not add BIL or introduce another ETF-pair proxy without a spec amendment. |
...
- Current truth: the checked generated YAML and report close the V1 fixture-window CPI/NFP gap in this repo. The current artifact contains `699` total events: `147` FOMC rows (`2007-10-31` through `2026-03-18`), `131` CPI rows (`2016-01-20` through `2026-12-10`), `131` NFP rows (`2016-01-08` through `2026-12-04`), plus ECB (88 decisions), BoE (96 decisions), BoJ (89 decisions) covering 2016-2026, and additional approved V2 rows for budget/election events. The source is the BLS yearly schedule structure via the repo-local archive-backed fetch path for FOMC/CPI/NFP, and official central-bank archive + current-calendar pages for ECB/BoE/BoJ. The event calendar is S3-only via manifest (not git-tracked for materialization); `default_relpath=("event_calendar", "us_events.yaml")` added to `ManifestInputSpec`. Provenance artifacts (`event_candidates.parquet`, `event_validations.parquet`, `event_quarantine.parquet`) are tracked in the manifest as audit-only entries.
...
- `BIL` — this ETF is **not** in the V2 cross-asset fragility universe by design. V2 §2C consumes the real ICE BofA OAS series from FRED directly and computes its approved parallel proxy from already-fetched `TLT`/`HYG`/`LQD`; **do not extend the universe** with BIL for another proxy without a spec amendment. `IEF` is now a §3.1 duration-diversification input only.
...
- If V2 §2C says **HY/IG credit spread**, read the real ICE BofA OAS series from FRED — `BAMLH0A0HYM2` (HY) and `BAMLC0A4CBBB` (BBB), both in `V2_FRED_SERIES` (Ambiguity Log #49). Keep the approved TLT-vs-HYG/LQD proxy as the separate `credit_funding_state_proxy` metric for longer directional history and route downstream rules through `credit_funding_effective_state` (Ambiguity Log #71 / ADR 0007). Do not add BIL or introduce another ETF-pair proxy without a spec amendment; `IEF` is §3.1-only.
- If V2 §2D says **`budget_week` / `election_window` / `geopolitical_event` / `global_rate_decision`**, use the event-source pipeline: deterministic/official adapters for scheduled/official rows and Group B candidate parquet for approval-gated rows. `geopolitical_event` may be generated from GPR, GDELT, and, after TODO API-key entitlement is resolved, ACLED plus Uppsala/UCDP evidence. It must stay overlay-only; do not auto-promote it from external news APIs, humanitarian aggregates, or LLM extraction. HDX HAPI monthly/admin aggregates are not candidate sources.
